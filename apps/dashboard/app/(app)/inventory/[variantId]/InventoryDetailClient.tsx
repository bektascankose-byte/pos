...
export function InventoryDetailClient({
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setMessage(null);
    // Captured synchronously -- `e.currentTarget` is nulled out by React
    // once this handler returns, so it can't be read from inside the async
    // transition callback below.
    const form = e.currentTarget;
    const formData = new FormData(form);
    startTransition(async () => {
      const result = await postAdjustmentAction(variantId, storeId, formData);
      if (result.ok) {
